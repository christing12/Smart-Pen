...
def main(start_time, file):
    ...
    plt.xlabel("Time") #name the x axis of the program
    plt.ylabel("Amplitude") #name the y axis of the program
    plt.title("Overview")
    ...
    sample_slice_fft = sample_slice_fft[range(0, max_frq_idx)]     #truncate the sample slice fft array so it goes from 0 to 2000 Hz
    ...
    plt.plot(frq, abs(sample_slice_fft)) #x values are the frequency values and the y values are the fft
    plt.xlabel("Frequency") #name the x axis of the program
    plt.ylabel("Amplitude") #name the y axis of the program
